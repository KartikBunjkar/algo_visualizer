...
class App extends Component {
	state = { //we have to use use state here
		array: [],
		arraySteps: [],
		colorKey: [],
		colorSteps: [],
	...
	componentDidMount() {
		window.addEventListener('load', riseText);//when load happens rise text will be called
		this.generateBars();
	}
	...
	generateSteps = () => {
		let array = this.state.array.slice();//use of slice is unknown since this return whole  array
		let steps = this.state.arraySteps.slice();
		let colorSteps = this.state.colorSteps.slice();

		this.ALGORITHMS[this.state.algorithm](array, 0, steps, colorSteps);//here we have passed value default bubble sort algorithm

		this.setState({ //here we are setting state for following parameters 
			arraySteps: steps,
			colorSteps: colorSteps,
		});
	};

	generateRandomNumber = (min, max) => { //from here we can take our input
	...
	generateBars = () => {
		...
		for (let i = 0; i < barCount; i++) {
			arr.push(this.generateRandomNumber(50, 200));// here we have to put our number
		}

		this.setState(
			{
				array: arr, // we have to look after this since upper array is going to be changed by us manually
				arraySteps: [arr], //look here too
				barCount: barCount,
				currentStep: 0,
			},
			() => this.generateSteps()
